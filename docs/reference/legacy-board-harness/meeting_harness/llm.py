    ...
    def request(
        ...
        *,
        ...
        try:
            ...
            atomic_write_text(request_dir / "response_http.txt", raw_http)
            raise LlmRunError(
                f"LLM HTTP response ended early after {len(exc.partial)} bytes"
            ) from exc
        except (urllib.error.URLError, TimeoutError, OSError) as exc:
            raise LlmRunError(f"LLM HTTP request failed: {exc}") from exc
        request_seconds = round(time.time() - request_started, 3)
        atomic_write_text(request_dir / "response_http.txt", raw_http)
        try:
            response = json.loads(raw_http)
        except json.JSONDecodeError as exc:
            raise LlmRunError(f"LLM HTTP body is not valid JSON: {exc}") from exc
        if not isinstance(response, dict):
            raise LlmRunError("LLM HTTP response must be a JSON object")
        atomic_write_json(request_dir / "response.json", response)

        choices = response.get("choices")
        if not isinstance(choices, list) or not choices or not isinstance(choices[0], dict):
            raise LlmRunError("LLM response has no first choice")
        choice = choices[0]
        message = choice.get("message")
        if not isinstance(message, dict):
            raise LlmRunError("LLM response choice has no message object")
