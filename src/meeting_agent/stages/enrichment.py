...
def extract_qa(
    ...
    """【受控生成式】要点问答（对标阿里"要点回顾"）：按段生成"关键问题+归纳答案"。

    弃用旧的"抽取相邻问答 turn"——会议口语很少有干净一问一答，抽出来全是散碎片段。
    改为 extract-then-compose（CLAUDE.md 原则2，4B 能扛）：就本段内容重新组织出通顺问句、
    基于本段事实归纳出书面答案。质量门仍在（去重/限量/挡半句），另加"答案不得照抄口语原话"。
    """
    sys = (
        "你是会议要点问答生成器。针对下面这段会议内容，提炼读者最关心的关键问题并作答。\n"
        "严格要求：\n"
        "- question：用完整通顺的书面问句重新表述，不要照抄原话的半句或口语；\n"
        "- answer：基于这段内容归纳出准确、通顺的书面答案（整合相关信息成一两句话，"
        "不要照搬口语原话或识别噪声，不要臆造这段没有的信息）；\n"
        "- 只就这段里实际讨论、且有明确结论或信息的内容提问；"
        "这段没有值得问的实质内容，就返回空数组；\n"
        "- 最多 3 条，按重要性排序。\n"
        '以JSON输出 {"qa":[{"question":"完整问句？","answer":"通顺书面答案"}]}。'
    )
    # 不用 few-shot：弱 4B 会 echo 示例。靠强 system 约束 + 质量门/去重/限量把控。
