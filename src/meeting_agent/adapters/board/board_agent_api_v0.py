...
except ImportError:  # direct script compatibility for the existing board deployment
    from board_harness_worker_v0 import (
        HARNESS_MODEL_PROFILE as WORKER_HARNESS_MODEL_PROFILE,
        HarnessRunConfig,
        HarnessWorkerCancelled,
...
def task_snapshot() -> dict[str, object] | None:
    ...
    with _task_lock:
        if _task is None:
            return None
        snapshot = dict(_task)
        for key in ("error", "harness_stage_details", "artifact_refs", "harness_diagnostics", "harness_metrics"):
            value = _task.get(key)
            if isinstance(value, dict):
                snapshot[key] = _bounded_task_value(value)
        return snapshot
...
def _bounded_task_value(value: object, depth: int = 0) -> object:
    """Keep nested Harness diagnostics small and JSON-safe."""
    ...
    if isinstance(value, dict):
        return {
            str(key)[:80]: _bounded_task_value(item, depth + 1)
            for key, item in list(value.items())[:24]
        }
    if isinstance(value, list):
        return [_bounded_task_value(item, depth + 1) for item in value[:16]]
    return None
...
def _safe_error_projection(value: object) -> dict[str, object]:
    """Keep stable error fields and exclude prompt/raw model content."""

    if not isinstance(value, dict):
        return {}
    projection = {
        key: _bounded_task_value(value[key])
...
def _safe_ref(value: object) -> str | None:
    """Return a short filename/reference instead of an absolute path."""
...
def _safe_artifact_refs(value: object) -> dict[str, object]:
    if not isinstance(value, dict):
        return {}
...
def _safe_runtime(value: object) -> dict[str, object] | None:
    if not isinstance(value, dict):
        return None
    allowed = ("run_id", "total_elapsed_seconds", "finished_at")
    return {key: value[key] for key in allowed if key in value}


def _safe_diagnostics(value: object) -> dict[str, object] | None:
    """Project canonical diagnostics without forwarding content fields."""
    ...
    error = _safe_error_projection(value.get("error"))
    if error:
        projection["error"] = error
    stage_details = value.get("stage_details")
    if isinstance(stage_details, dict):
        projection["stage_details"] = _safe_stage_details(stage_details)
    refs = _safe_artifact_refs(value.get("artifact_refs"))
    if refs:
...
def _safe_stage_details(details: dict[str, object]) -> dict[str, object]:
    """Keep bounded nested stage details in the task snapshot."""

    projection: dict[str, object] = {}
    for key in ("status", "return_code", "elapsed_seconds", "diagnostic_source"):
        if key in details:
            projection[key] = _bounded_task_value(details[key])
    for key in ("log", "result_path", "worker_log"):
        safe = _safe_ref(details.get(key))
        if safe:
            projection[key] = safe
    error = _safe_error_projection(details.get("error"))
    if error:
        projection["error"] = error
    diagnostics = _safe_diagnostics(details.get("diagnostics"))
    if diagnostics:
        projection["diagnostics"] = diagnostics
    metrics = _safe_metrics(details.get("metrics"))
    if metrics:
        projection["metrics"] = metrics
    identity = details.get("identity")
    if isinstance(identity, dict):
        projection["identity"] = _bounded_task_value(identity)
    refs = _safe_artifact_refs(details.get("artifact_refs"))
    if refs:
        projection["artifact_refs"] = refs
    return projection
...
def mark_harness_failed(
    ...
    with _task_lock:
        ...
        worker_error_dict = worker_error if isinstance(worker_error, dict) else {}
        nested_error = worker_error_dict.get("error")
        nested_error = nested_error if isinstance(nested_error, dict) else {}
        failed_stage = str(
            worker_result.get("stage")
            or nested_error.get("stage")
            or worker_error_dict.get("stage")
            or _task.get("stage")
            or "harness"
        )
        if failed_stage == "processing":
            failed_stage = "harness"
        failure_code = str(
            nested_error.get("code")
            or worker_error_dict.get("code")
            or code
        )[:96]
        failure_message = str(
            nested_error.get("message")
            or worker_error_dict.get("message")
            or message
        )[:600]
        _task["state"] = "failed"
        _task["stage"] = failed_stage
        _task["seq"] = int(_task["seq"]) + 1
        _task["updated_at"] = utc_now()
        safe_diagnostics = _safe_diagnostics(worker_result.get("diagnostics"))
        safe_artifact_refs = _safe_artifact_refs(worker_result.get("artifact_refs"))
        safe_worker_log = _safe_ref(worker_result.get("worker_log"))
        safe_result_path = _safe_ref(worker_result.get("result_path"))
        _task["error"] = {
            "code": failure_code,
            "message": failure_message,
            "stage": failed_stage,
            "wrapper_code": str(code)[:96],
            "wrapper_message": str(message)[:600],
            "return_code": worker_result.get("return_code"),
            "elapsed_seconds": worker_result.get("elapsed_seconds"),
            "run_id": _bounded_task_value(worker_result.get("run_id")),
            "runtime": _safe_runtime(worker_result.get("runtime")),
            "diagnostic_source": worker_result.get("diagnostic_source"),
            "diagnostics": safe_diagnostics,
            "metrics": _safe_metrics(worker_result.get("metrics")),
            "identity": _bounded_task_value(worker_result.get("identity")),
            "worker_log": safe_worker_log,
            "result_path": safe_result_path,
            "artifact_refs": safe_artifact_refs,
        }
        _task["harness_result_path"] = safe_result_path
        _task["artifact_refs"] = safe_artifact_refs
        _task["worker_log"] = safe_worker_log
        _task["harness_elapsed_seconds"] = worker_result.get("elapsed_seconds")
        _task["harness_stage_details"] = _safe_stage_details(
            {
                "status": "failed",
                "return_code": worker_result.get("return_code"),
                "elapsed_seconds": worker_result.get("elapsed_seconds"),
                ...
                "diagnostics": safe_diagnostics,
                "metrics": _safe_metrics(worker_result.get("metrics")),
                "identity": worker_result.get("identity"),
                "artifact_refs": safe_artifact_refs,
                "result_path": safe_result_path,
                "worker_log": safe_worker_log,
            }
...
def run_harness_worker(
    ...
    try:
        current = task_snapshot() or {}
        worker_config = HarnessRunConfig(
            trace_id=task_id,
            meeting_id=str(current.get("meeting_id") or task_id),
            task_id=task_id,
        )
        try:
        ...
        with _task_lock:
            ...
            _task["harness_result_path"] = _safe_ref(worker_result.get("result_path"))
            _task["artifact_refs"] = _safe_artifact_refs(worker_result.get("artifact_refs"))
            _task["harness_diagnostics"] = _safe_diagnostics(worker_result.get("diagnostics"))
            _task["harness_metrics"] = _safe_metrics(worker_result.get("metrics"))
            _task["diagnostic_source"] = worker_result.get("diagnostic_source")
            _task["worker_log"] = _safe_ref(worker_result.get("worker_log"))
            _task["harness_elapsed_seconds"] = worker_result.get("elapsed_seconds")
